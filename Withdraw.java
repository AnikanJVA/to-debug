public class Withdraw{ // kani nga class kay para sa tanan related sa withdraw 
    private File withdrawFile; // this is the File variable used to access/read the text file 
    // Unsa atong need para ma gamit ang File nga class

    public { // this is the constructor that access/read the withdraw text file of specific accounts
        = new File( + "_withdraws.txt"); // unsa na variable ang need para ma access/read ang file
    }                                    // unsa atong need ibutang para unique ang deposit file sa isa ka account

                // unsa na data type ang dapat dawaton sa arraylist
    public ArrayList<> getWithdrawList(){ // this method puts the contents of the text file into an arraylist
        ArrayList<Transaction> withdrawList = ; // unsa ang need para mag declare ug arraylist
        try{
        ...
        return ; // unsa ang dapat i-return diri
    }

    public void AmountWithdraw(double amount){  // this method adds a withdraw transaction into the file
        ArrayList<> withdrawList =  // dapat ang sulod sa arraylist kay ang tanan withdraw sa account nga naa sa text file
        FileWriter fw
        BufferedWriter bw
        int transactionId = 0;
        ...
        try{ // this part writes the new deposit transaction into the file
            = new FileWriter(withdrawFile, true); // unsa ang names sa atong variable nga FileWriter
